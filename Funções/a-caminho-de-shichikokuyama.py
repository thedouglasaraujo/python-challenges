...
while parar != True:
    ...
    if len(temporario) == 1:
        print(
            f"Lembrei! A sílaba {juntarstr} está no nome do hospital. Obrigada, Totoro!")
    if len(temporario) == len(silabas) and len(temporario) > 1 and letras in "shichikokuyama":
        print(
            f"A palavra {letras} está toda no nome do hospital. Acertou em cheio, Totoro!")
        Fazer = False
    if len(temporario) >= 2 and Fazer == True:
        print(
            f"Lembrei! As sílabas: {juntarstr} estão no nome do hospital. Obrigada, Totoro!")
    limparlistas()
    Fazer = True
