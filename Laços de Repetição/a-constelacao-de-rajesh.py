...
while (contador != estrelas) and (estrelas >= 3):
  ...
  eixo_y = int(input())
  contador += 1 
  if ((contador%2) != 0) and (contador != 1): #Se o contador for 1, ele não consegue comparar a distância com outra estrela
    distancia = int(((eixo_x2 - eixo_x)**2 + (eixo_y2 - eixo_y)**2)**(1/2))
    print("Distância [star"+ str((contador-1)) + " <-> star" + str((contador)) + "]: " + str(distancia))
    soma += distancia 
