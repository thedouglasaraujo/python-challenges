...
parar = True
Mestre = False
adversarios_conhecidos = {'Vingador': 30,
                          'Tiamat': 20, 'Vingador das Sombras': 14}
grupo = {'Bobby': ('Grande espada', 8, 'media'), 'Diana': ('Dardo', 12, 'leve'), 'Eric': ('Grande espada', 8, 'pesada'), 'Hank': (
    'Espada', 6, 'media'), 'Presto': ('Cajado', 4, 'leve'), 'Sheila': ('Espada', 6, 'leve'), 'Uni': ('Chifre', 2, 'leve')}
if adversario in adversarios_conhecidos:
    # Pontos de vida dos adversários conhecidos
    pontos_de_vida = adversarios_conhecidos.get(adversario)
else:
    pontos_de_vida = 9
...
while parar == True:
    try:
        ...
        if pontos_de_vida <= 0 and Mestre == False:
            print(
                f"{personagem} executou o ultimo golpe em {adversario}, estamos livres!")
