...
def multiplicar(lista1, lista2): #Função para multiplicar Fatorial e Fibonacci caso a operação 1 seja ímpar
  resultado = [elemA * elemB for elemA, elemB in zip(lista1, lista2)]
  return resultado
...
def fazer_mod(indice):
  numero = letras.index(palavra[indice]) #Analisa letra por letra da palavra 
  operacao1 = numero%11
  contador2 = 0
  ...
  if indice+1 < len(palavra):
    fatoracoes.clear()
    x.clear()
    fazer_mod(indice+1) #Recursão para analisar a próxima letra da palavra
...
while contador != qtd_palavras:
  ...
  for lista in senhanova:
    traducaosenha(0)
  juntarstr = ''.join(decifrar) #Formatação para verificar a igualdade da senha
